...
linux-secret-helper:
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -ldflags "-s -w -X main.Version=${VERSION}" -o bin/releases/linux-amd64/secret-helper main.go
	tar -C bin/releases/linux-amd64 -cvzf bin/releases/linux-amd64-${VERSION}.tgz secret-helper 

windows-secret-helper:
	GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -ldflags "-s -w -X main.Version=${VERSION}" -o bin/releases/windows-amd64/secret-helper.exe main.go
